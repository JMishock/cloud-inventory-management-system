...
//        product.getParts().addAll(assparts);
            ...
            if(product.getId()!=0) {
                ...
                if(product.getInv() - product2.getInv() > 0) {
                    int inventoryChange = product.getInv() - product2.getInv();

                    for (Part p : product2.getParts()) {
                        if(p.getInv() - inventoryChange < p.getMin()) {
                            theModel.addAttribute("product", product2);
                            theModel.addAttribute("parts", partService.findAll());
                            ...
                            theModel.addAttribute("availparts", availParts);
                            theModel.addAttribute("assparts", product2.getParts());
                            theModel.addAttribute("errorMessage", "Product update failed: associated part inventory would fall below minimum.");
                            return "productForm";
                        }
                    }
